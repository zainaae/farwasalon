...
function ConfirmationContent() {
  ...
  /* After a fresh Book → Confirm, open WhatsApp once with the prefilled
     message. Popup blockers may stop window.open — the green button stays.
     Skip automated browsers so e2e is not yanked into wa.me. */
  useEffect(() => {
    if (!loaded || !valid || cancelled || !openWa || !id || !waUrl) return
    if (typeof navigator !== 'undefined' && navigator.webdriver) return

    const storageKey = `farwa-wa-opened:${id}`
    try {
    ...
    const timer = window.setTimeout(() => {
      ...
      setWaHint(
        popup
          ? 'WhatsApp opened — tap Send so we both keep a copy.'
          : 'Tap the green button below to open WhatsApp, then tap Send.',
      )
      try {
        const next = new URL(window.location.href)
        next.searchParams.delete('openWa')
        window.history.replaceState({}, '', `${next.pathname}${next.search}`)
      } catch {
        /* ignore */
      }
  ...
  }, [loaded, valid, cancelled, openWa, id, waUrl])
      ...
      <m.div className="section-shell section-pad min-h-screen flex items-start justify-center">
        ...
        >
          ...
          <div role="status" aria-live="polite">
            ...
            {waHint ? (
              <p className="text-ink text-xs font-[family-name:var(--font-inter)] font-medium mb-6" aria-live="polite">
                {waHint}
              </p>
            ) : null}
          </div>
          ...
          <div className="flex flex-col gap-3 mb-8">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              onClick={() => setWaHint('WhatsApp opened — tap Send so we both keep a copy.')}
            ...
            <p className="text-stone text-[10px] font-[family-name:var(--font-inter)] font-light leading-relaxed">
              Message is ready — you only need to tap Send in WhatsApp.
            </p>
          </div>
          ...
          <div className="panel-soft p-6 text-left mb-8 shadow-soft">
            ...
            <div className="space-y-3">
              ...
              <div className="h-px bg-border-soft" />
              <div className="flex justify-between items-start">
                <span className="text-stone text-xs font-[family-name:var(--font-inter)]">Name</span>
                <span className="text-ink text-sm font-[family-name:var(--font-inter)] font-medium">{name}</span>
              </div>
              {id && (
                <>
                  <div className="h-px bg-border-soft" />
          ...
          {!durable && (
            <p className="mb-6 text-left text-[11px] text-stone font-[family-name:var(--font-inter)] leading-relaxed border border-border-soft bg-mist px-4 py-3">
          ...
          <div className="flex flex-col gap-3">
            ...
            <Link
              href="/book"
              className="tap-safe w-full inline-flex items-center justify-center gap-2 border border-border-soft text-ink text-[11px] tracking-[0.16em] uppercase font-semibold font-[family-name:var(--font-inter)] px-6 py-3.5 hover:bg-mist transition-colors"
            >
              Book Another Appointment
            </Link>
          </div>
          ...
          {durable && (
            <p className="mt-3 text-stone text-[10px] font-[family-name:var(--font-inter)] font-light">
          ...
          <div className="mt-8 pt-6 border-t border-border-soft">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 text-stone text-xs font-[family-name:var(--font-inter)] hover:text-ink transition-colors"
            >
