 ...
 * cancellations. Durable storage + a keepable WhatsApp copy close that gap
 * without putting the cancel token in the URL (analytics transmit href).
 *
...
/**
 * Prefer a non-empty cancel token from the API response; otherwise keep the
 * token already stored for this booking. Idempotent retries return
 * cancelToken: null — wiping the stored token would strand the customer
 * without a cancel link.
 */
export function resolveStoredCancelToken(incoming, existingToken = '') {
  const next = typeof incoming === 'string' ? incoming.trim() : ''
  if (next) return next
  return typeof existingToken === 'string' ? existingToken : ''
}
