...
describe('battle-test: bridal shares salon station cap (MAX_WORKERS=2)', () => {
  it('bridal maxWorkers matches the salon station count', () => {
    if (!bridal) return
    expect(getServiceMaxWorkers(bridal)).toBe(MAX_WORKERS)
  })

  it('with 2 concurrent bookings at 12:00, bridal cannot fit (pre-check capacity)', () => {
    if (!bridal) return
    const existing = [
      mkBooking({ time: '12:00', duration: 30 }),
      mkBooking({ time: '12:00', duration: 30 }),
    ...
    expect(r.fits).toBe(false)
    expect(r.maxWorkers).toBe(MAX_WORKERS)
  })
})
