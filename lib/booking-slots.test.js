...
describe('shouldCancelSelf (deterministic earliest-wins tiebreak)', () => {
  const row = (bookingId, { at = '2026-07-20T09:00:00Z', time = '12:00', status = 'Confirmed' } = {}) => ({
    bookingId,
    bookedAt: at,
    timeSlot: time,
    duration: 30,
  ...
  it('does not cancel when self is the only row', () => {
    const self = row('SELF')
    expect(shouldCancelSelf([self], self)).toBe(false)
  })

  it('cancels self when the station is genuinely full of strictly-earlier rows', () => {
    const self = row('SELF', { at: '2026-07-20T09:00:10Z' })
    const earlier = [row('A', { at: '2026-07-20T09:00:01Z' }), row('B', { at: '2026-07-20T09:00:02Z' })]
    expect(shouldCancelSelf([...earlier, self], self)).toBe(true)
  })

  it('keeps self when an equal-number of earlier rows is below capacity', () => {
    const self = row('SELF', { at: '2026-07-20T09:00:10Z' })
    const earlier = [row('A', { at: '2026-07-20T09:00:01Z' })]
    expect(shouldCancelSelf([...earlier, self], self)).toBe(false)
  })

  it('earliest racer survives a two-way race on the last station (both passed pre-check)', () => {
    // Station has 1 pre-booked row (1-of-2 full). Both racers pass their
    // pre-check against that snapshot, then both append → 3 rows on a 2-worker
    // station. Earliest-wins must leave exactly the earlier racer + the
    // pre-booked row; the later racer must cancel itself.
    const prebooked = row('PRE', { at: '2026-07-20T08:00:00Z' })
    const earlier = row('EARLY', { at: '2026-07-20T09:00:01Z' })
    const later = row('LATE', { at: '2026-07-20T09:00:05Z' })
    const rows = [prebooked, earlier, later]
    expect(shouldCancelSelf(rows, earlier)).toBe(false)
    expect(shouldCancelSelf(rows, later)).toBe(true)
  })

  it('manual/legacy rows (empty bookedAt) sort first and are never evicted', () => {
    const manual = row('MANUAL', { at: '' })
    const self = row('SELF', { at: '2026-07-20T09:00:01Z' })
    // One manual + self = 2 rows on a 2-worker station → self still fits.
    expect(shouldCancelSelf([manual, self], self)).toBe(false)
    // Two manual rows + self = 3 rows → self is the one that must go.
    const manual2 = row('MANUAL2', { at: '' })
    expect(shouldCancelSelf([manual, manual2, self], self)).toBe(true)
    // A manual row is never cancelled — it sorts first, so nothing is earlier.
    expect(shouldCancelSelf([manual, manual2, self], manual)).toBe(false)
  })

  it('uses a single shared capacity regardless of the racing services maxWorkers', () => {
    // The cross-service race that overbooked a 2-worker station: a threading
    // request (cap 2) and a bridal request (cap 3) both pre-checked "fits".
    // Each racer's own service cap must NOT be allowed to let it survive a
    // station that is genuinely full of earlier rows — a shared MAX_WORKERS
    // threshold makes both racers reach the same verdict.
    const self = row('BRIDAL', { at: '2026-07-20T09:00:05Z' })
    const earlier = [row('A', { at: '2026-07-20T09:00:01Z' }), row('B', { at: '2026-07-20T09:00:02Z' })]
    expect(shouldCancelSelf([...earlier, self], self)).toBe(true)
  })

  it('cancelled rows do not consume tiebreak capacity', () => {
    const self = row('SELF', { at: '2026-07-20T09:00:05Z' })
    const cancelled = [row('X', { at: '2026-07-20T09:00:01Z', status: 'Cancelled' })]
    expect(shouldCancelSelf([...cancelled, self], self)).toBe(false)
  })

  it('rows on non-overlapping slots never count against self', () => {
    const self = row('SELF', { at: '2026-07-20T09:00:05Z' })
    const other = row('A', { at: '2026-07-20T09:00:01Z', time: '17:00', endTime: '17:30' })
    expect(shouldCancelSelf([other, self], self)).toBe(false)
  })
})
