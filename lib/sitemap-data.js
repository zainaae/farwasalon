...
import { PRIORITY_LOCATION_SLUGS, NEIGHBORHOODS } from '../src/location-seo.js'
...
export const LAST_SERVICE_UPDATE = '2026-07-31' // src/data.js — menu + prices
export const LAST_CONTENT_UPDATE = '2026-07-28' // src/faq-data.js — FAQ copy
export const LAST_AREA_UPDATE = '2026-07-30' // src/area-content.js — per-area pages
...
export function getStaticSitemapEntries(now = new Date()) {
  ...
  return [
    ...
    { url: `${SITEMAP_BASE}/privacy`, lastModified: '2026-08-03', changeFrequency: 'yearly', priority: 0.3 },
  ]
}
...
export function getLocationSitemapEntries() {
  const hubs = PRIORITY_LOCATION_SLUGS.map((slug) => ({
    url: `${SITEMAP_BASE}/services/${slug}`,
    lastModified: LAST_SERVICE_UPDATE,
    changeFrequency: 'monthly',
    priority: 0.7,
  }))
  /* Per-area pages: one per neighbourhood, subject is the area rather than the
     service. Higher priority than the service hubs because these carry the
     local intent the retired matrix was failing to. */
  const areas = NEIGHBORHOODS.filter((n) => AREA_CONTENT[n.slug]).map((n) => ({
    url: `${SITEMAP_BASE}/areas/${n.slug}`,
    lastModified: LAST_AREA_UPDATE,
    changeFrequency: 'monthly',
    priority: 0.75,
  }))
  return [...hubs, ...areas]
...
export function getSitemapIndexEntries(now = new Date()) {
  ...
  return [
    ...
    { loc: `${SITEMAP_BASE}/sitemap-locations.xml`, lastModified: newest(getLocationSitemapEntries()) },
  ]
}
