...
describe('resolveStoredCancelToken', () => {
  it('keeps a non-empty response token', async () => {
    const { resolveStoredCancelToken } = await loadModule()
    expect(resolveStoredCancelToken('new.token', 'old.token')).toBe('new.token')
    expect(resolveStoredCancelToken('  new.token  ', 'old.token')).toBe('new.token')
  })

  it('preserves the stored token when the response token is null/empty', async () => {
    const { resolveStoredCancelToken } = await loadModule()
    expect(resolveStoredCancelToken(null, 'old.token')).toBe('old.token')
    expect(resolveStoredCancelToken('', 'old.token')).toBe('old.token')
    expect(resolveStoredCancelToken('   ', 'old.token')).toBe('old.token')
    expect(resolveStoredCancelToken(undefined, 'old.token')).toBe('old.token')
  })

  it('returns empty when neither side has a token', async () => {
    const { resolveStoredCancelToken } = await loadModule()
    expect(resolveStoredCancelToken(null, '')).toBe('')
    expect(resolveStoredCancelToken('', undefined)).toBe('')
  })
})
