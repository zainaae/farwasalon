...
vi.mock('./whatsapp-cloud.js', () => ({
  maybeSendBookingConfirmed: vi.fn(async () => ({ ok: false, skipped: true })),
}))
...
/* clientName defaults to the request's name because a real sheet row always has
   one — the route requires it — and these fixtures represent the SAME person
   retrying. Tests about a different guest override it explicitly. */
function confirmed(time, duration = 30) {
  return {
    ...
    clientName: VALID_BODY.clientName,
  }
}
...
beforeEach(() => {
  ...
  /* true = a row was actually written. The mock used to resolve undefined, which
     is indistinguishable from the silent no-op the real function returns when
     auth is missing or the row is not found — the reason a cancel that wrote
     nothing was reported to the customer as success. */
  updateBookingStatus.mockReset().mockResolvedValue(true)
...
describe('POST /api/book - double-booking guard', () => {
  ...
  it('two racers on the last free station: exactly one wins, one rolls back', async () => {
    ...
    updateBookingStatus.mockImplementation(async (bookingId, status) => {
      ...
      if (!row) return false // mirrors the real rowIndex === -1 path
      row.status = status
      return true
    })
  ...
  /* The sheet may hold a differently-cased or -spaced version of the same name.
     A genuine retry still dedupes, and the response must echo what the client
     sent rather than the sheet's copy — the sheet value is PII the retry
     already knows, and returning it on an unauthenticated phone+slot match
     would leak it. The fixture used to differ in the NAME itself, which
     encoded "a second guest is a retry"; see the two-guests test below. */
  it('idempotent response echoes the request clientName, never the sheet name', async () => {
    const existing = {
      ...confirmed('12:00'),
      bookingId: 'FBS-DEDUPE-NAME',
      clientPhone: '0322 2782254',
    ...
    getSheetRows.mockResolvedValue([existing])
    const res = await POST(makeRequest({ ...VALID_BODY, clientName: 'Retry Client' }))
    expect(res.status).toBe(200)
    const { booking } = await res.json()
    expect(booking.clientName).toBe('Retry Client')
    expect(booking.clientName).not.toBe('Sheet Stored Name')
  })

  it('bridal with 2 Confirmed on the slot is 409 at pre-check (no write-then-cancel)', async () => {
    ...
    getSheetRows.mockResolvedValue(earlier)
    const res = await POST(
      makeRequest({
        ...VALID_BODY,
        serviceId: bridal.id,
        clientPhone: '03003333333',
      }),
    )
    expect(res.status).toBe(409)
    expect(appendBooking).not.toHaveBeenCalled()
...
/* The idempotency key originally matched on phone + slot + service and ignored
   the name. A mother booking threading at 15:00, then booking the same slot for
   her daughter on the same phone, looked identical to a retry: the second
   request returned the mother's bookingId, never wrote a row, and the daughter
   was turned away — while MAX_WORKERS is 2 and the slot genuinely had room.
   Worse, the response echoed the daughter's name against the mother's id, so
   the client overwrote the mother's stored record. */
describe('two guests on one phone are two bookings, not a retry', () => {
  it('books the second guest at the same slot on the same phone', async () => {
    const mother = {
      ...confirmed('12:00'),
      bookingId: 'FBS-MOTHER01',
      clientPhone: '0322 2782254',
      clientName: 'Ayesha Khan',
      service: SERVICE.name,
      bookedAt: new Date(Date.now() - 120_000).toISOString(),
    }
    getSheetRows.mockResolvedValue([mother])

    const res = await POST(makeRequest({ ...VALID_BODY, clientName: 'Sara Khan' }))
    expect(res.status).toBe(200)
    const { booking } = await res.json()
    expect(booking.id, 'must not return the first guest booking id').not.toBe('FBS-MOTHER01')
    expect(appendBooking, 'the second guest needs her own row').toHaveBeenCalled()
    expect(booking.cancelToken, 'a real booking gets a real token').toBeTruthy()
  })

  it('still dedupes a true retry — same name, however it is spaced or cased', async () => {
    const first = {
      ...confirmed('12:00'),
      bookingId: 'FBS-RETRY001',
      clientPhone: '0322 2782254',
      clientName: 'AYESHA   khan',
      service: SERVICE.name,
      bookedAt: new Date(Date.now() - 60_000).toISOString(),
    }
    getSheetRows.mockResolvedValue([first])

    const res = await POST(makeRequest({ ...VALID_BODY, clientName: 'Ayesha Khan' }))
    expect(res.status).toBe(200)
    expect((await res.json()).booking.id).toBe('FBS-RETRY001')
    expect(appendBooking).not.toHaveBeenCalled()
  })
})
