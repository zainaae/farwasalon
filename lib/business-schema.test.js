...
describe('buildBeautySalonSchema', () => {
  it('builds BeautySalon JSON-LD', () => {
    ...
    expect(s['@type']).toEqual(['BeautySalon', 'LocalBusiness'])
    expect(s.telephone).toBe(SALON_PHONE)
    expect(s.aggregateRating).toBeDefined()
    expect(s.paymentAccepted).toContain('JazzCash')
  })

  it('includes OfferCatalog entries for every CAT_SLUGS category', async () => {
    const { CAT_SLUGS } = await import('../src/data.js')
    const s = buildBeautySalonSchema()
    const names = s.hasOfferCatalog.itemListElement.map((c) => c.name)
    for (const cat of Object.keys(CAT_SLUGS)) {
      expect(names).toContain(cat)
    }
    expect(names.length).toBe(Object.keys(CAT_SLUGS).length)
  })
})
