 ...
 *
 * A `fromPrice` service — hair, hair treatments, and bridal looks that include
 * hair, where length and density genuinely decide the final cost — must never
 * be emitted as `price`. That publishes a fixed figure the salon has not agreed
 * to and that the page itself does not claim: the menu says "from Rs 4,000" and
 * an assistant quoting "Rs 4,000" would be misquoting it. schema.org's construct
 * for a floor is a PriceSpecification carrying `minPrice` and no `price`, so a
 * parser reads "at least Rs 4,000" instead of "Rs 4,000 exactly".
 */
export function buildServiceOffer(service, { category } = {}) {
  ...
  if (service.fromPrice) {
    ...
    offer.description =
      'Starting price (quote floor). Hair length and density decide the final cost, which is confirmed before any work begins.'
  } else {
    ...
    '@graph': [
      { '@id': SALON_ID, hasOfferCatalog: { '@id': PRICE_CATALOG_ID } },
      {
        ...
        description:
          ...
          `${stats.fixed} are fixed rates; ${stats.startingFrom} hair, hair-treatment, and bridal services publish a starting price ` +
