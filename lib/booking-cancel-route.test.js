...
/* updateBookingStatus returns false when it wrote nothing — missing auth, or a
   bookingId absent from the sheet. Awaiting it in a try/catch caught neither,
   so the customer saw "Cancelled" while the row stayed Confirmed: the salon
   held her station and she did not arrive. */
describe('a cancel that writes nothing is not a success', () => {
  it('returns 502 rather than telling the customer she is cancelled', async () => {
    updateBookingStatus.mockResolvedValueOnce(false)
    const res = await POST(makeRequest({ token: tokenFor() }))
    expect(res.status).toBe(502)
    expect((await res.json()).error).toMatch(/could not cancel|WhatsApp/i)
  })

  it('still succeeds when the row is genuinely written', async () => {
    updateBookingStatus.mockResolvedValueOnce(true)
    const res = await POST(makeRequest({ token: tokenFor() }))
    expect(res.status).toBe(200)
    expect(await res.json()).toEqual({ success: true, bookingId: BOOKING.bookingId })
  })
