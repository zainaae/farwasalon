...
/**
 * Parse one Bookings sheet data row (A–N) into a plain object.
 * Shared by public slot reads and POS import — shape must stay stable.
 * @param {string[]} row
 */
export function parseBookingSheetRow(row) {
...
/**
 * All Bookings rows (excluding header). Additive helper for POS import.
 * Does not change public /api/book behaviour.
 * @returns {Promise<ReturnType<typeof parseBookingSheetRow>[]>}
 */
export async function listAllBookingRows() {
  const rows = await fetchAllBookingRows()
  if (rows.length <= 1) return []
  return rows.slice(1).map(parseBookingSheetRow)
}
