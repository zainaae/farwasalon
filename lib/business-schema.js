...
/** Sync with live Google rating when possible (env overrides defaults). */
export function getAggregateRating() {
  const ratingValue =
    process.env.SALON_GBP_RATING || process.env.NEXT_PUBLIC_SALON_GBP_RATING || '4.9'
  const reviewCount =
  ...
  const graph = [
    {
      ...
      description: `${service.description} Serving ${location.name}, Karachi.`,
...
export function buildHeroVideoSchema() {
  return {
    ...
    contentUrl: `${SITE_ORIGIN}/hero-mp4.mp4`,
    embedUrl: `${SITE_ORIGIN}/`,
    uploadDate: `${FOUNDING_YEAR}-01-01`,
    duration: 'PT30S',
    inLanguage: 'en',
    publisher: { '@id': SALON_ID },
  }
