...
describe('Freedom Deal lifecycle', () => {
  ...
  it('teases before it opens, runs during, and disappears after', () => {
    ...
    expect(getActiveDeals(at('2026-08-10')).map((d) => d.id)).toContain('freedom-deal-2026')
...
describe('deals page after a campaign ends', () => {
  /* first-facial-10 was retired (no honour path). An empty /deals grid after
     Freedom Deal ends is honest — the page renders an empty state, not a
     fake evergreen % offer. */
  it('has no evergreen filler deals', () => {
    const evergreen = DEALS.filter((d) => d.validUntil === null)
    expect(evergreen.length).toBe(0)
  })
